...
const TrainPage = () => {
    ...
    const getCartSelect = async () => {
        const resp2 = await Train.getDisponibleCarts();
        setCartSelect(resp2);
    }

    const getTrains = async () => {
        const resp2 = await Train.getTrains();
        setTrainList(resp2);
    }
    ...
    const addTrain = () => {
        if(trainCode && idLine && selectedCarts.length>0){
            let data = {
                'code': trainCode,
                'id_line': idLine,
                'carts': selectedCarts
            }
            Train.addTrain(data).then( (res) => {
                if(res.id_inserted){
                    Swal.fire({
                        position: 'center',
                        icon: 'success',
    ...
    const handleShowEdit = (row) => {
        setTrainCode(row.code);
        setIdLine(row.id_line);
        setIdTrain(row.id_train);
        Train.getModalTrain(row.id_train).then(res => setCartsList(res));
        Train.selectedCarts(row.id_train).then(res => setSelectedCarts(res));
        setShowEdit(true);
    }
    ...
    const handleSubmitEdit = () => {
        if(trainCode && idLine && selectedCarts.length>0){
            ...
            console.log(data)
            Train.updateTrain(data).then( (res) => {
                if(res.id_updated){
                    Swal.fire({
                        position: 'center',
                        icon: 'success',
    ...
    const deleteTrain = (e) => {
        Swal.fire({
            title: 'Estas seguro de Eliminar este Tren?',
            ...
            if (result.value) {
                Train.deleteTrain(e.id_train).then( res => {
                if(res.id_deleted){
                    Swal.fire(
                    'Eliminado!',
                    'El Tren ha sido eliminado.',
                    'success'
                    )
                    getTrains();
                    getCartSelect();
                        ...
                        <div className="col-md-12">
                            <div className="panel panel-inverse">
                                <div className="panel-heading">
                                    ...
                                    <h4 className="panel-title">Registrar Tren</h4>
                                </div>
                                <div className="panel-body panel-form">
                                    <form className='form-horizontal form-bordered' onSubmit={addTrain}>
                                        <fieldset>
                                            <div className='row'>
                                                <div className='col-md-6'>
                                                    <div className="form-group">
                                                        <label className="control-label col-md-4">Código</label>
                                                        <div className="col-md-8">
                                                            <input type="text" className="form-control" id="code" placeholder="" value={trainCode} onChange={(e) => noCharacter(e.target.value)}/>
                                                        </div>
                                                    </div>
                                                    <div className="form-group">
                                                        <label className="control-label col-md-4">Carritos</label>
                                                        <div className="col-md-8">
                                                            <MultiSelect
                                                                options={cartSelect}
                                                                value={selectedCarts}
                                                                hasSelectAll={false}
                                                                onChange={setSelectedCarts}
                                            ...
                                            <div className='form-group'>
                                            <center>
                                                <button type="button" className="btn btn-sm btn-primary m-r-5" onClick={addTrain}>Agregar</button>
                                        ...
                                        <Table sx={{ minWidth: 650 }} stickyHeader aria-label="sticky table">
                                            ...
                                            >
                                                <TableRow>
                                                    ...
                                                    <TableCell><h6><strong>Código</strong></h6></TableCell>
                                                    <TableCell><h6><strong>Area</strong></h6></TableCell>
                                                    <TableCell><h6><strong>Linea</strong></h6></TableCell>
                                                    <TableCell><h6><strong>Carros</strong></h6></TableCell>
                                                    ...
                                                    <TableRow key={row.id_train}>
                                                        ...
                                                        <TableCell align="center">                                                                        
                                                            <div className="align-left">
                                                                ...
                                                                <Button  variant="contained" className="btn-sm btn-danger" title="Eliminar" onClick={() => deleteTrain(row)}>
                            ...
                            <div className="row">
                                <div className="col-md-6">
                                    <div className="form-group">
                                        ...
                                        <input type="text" className="form-control" id="trainCode" placeholder=""  value={trainCode} onChange={ (e) => noCharacter(e.target.value) } />
                                    </div>
                                    <div className="form-group">
                                        <label className="form-label"> Linea:{' '}</label>
                                        <input type="number" className="form-control" id="idLine" placeholder=""  value={idLine} onChange={ (e) => setIdLine(e.target.value) } />
                                    </div>
                                    <div className="form-group">
                                        <label className="form-label">Carritos:</label>
