...
$.widget("bootstrap.bwizard", {
	...
	_createButtons: function () {
		...
		if (!this.buttons) {
			...
			this.buttons = $('<ul className="pager"/>');
			this.buttons.addClass('bwizard-buttons');
			if(backBtnText != ''){
				this.backBtn =
					$("<li className='previous'><a href='#'>" +
						backBtnText + "</a></li>")
	...
	_pageLize: function (init) {
		...
		if (this.list) {
			...
			this.lis.each(function(i){
				if (o.clickableSteps){
					...
					$(this).contents().wrap('<a href="#step' + (i+1) + '" className="hidden-phone"/>');
				} else {
					$(this).contents().wrap('<span className="hidden-phone"/>');
				}
				$(this).attr("role", "tab")
				$(this).css('z-index',self.lis.length-i);
				$(this).prepend('<span className="label">' + (i+1) + '</span>');
				if (!isOL) {
		...
		if (init) {
			...
			// 1. from cookie
			// 2. from actived className attribute on panel
	...
	load: function (index) {
		...
		if (o.spinner) {
			...
			if (!spinner) {
				spinner = $('<div className="modal" id="spinner" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"/>');
